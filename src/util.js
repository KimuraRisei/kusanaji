...
// Romaji conversion tables (hoisted to module level for performance —
// creating 1200 object properties per toRawRomaji() call was the biggest
// CPU bottleneck in the entire project).
    const romajiSystem = {
        nippon: {
            // 数字と記号
            "１": "1",
 ...
 */
// String type constants used by the okurigana/furigana code path
const STR_TYPE = { KANJI: 0, MIXED: 1, KANA: 2, OTHER: 3 };
...
const getStrType = function (str) {
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        if (hasKJ && hasHK) return STR_TYPE.MIXED; // early exit
    }
    if (hasKJ) return STR_TYPE.KANJI;
    if (hasHK) return STR_TYPE.KANA;
    return STR_TYPE.OTHER;
};
